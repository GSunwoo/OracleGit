...
형변환함수 / 기타함수
설명 : 자료형을 다른 타입을 변환해야 할때 사용하는 함수와 기타함수
***/

/*
sysdate : 현재날짜와 시간을 초 단위로 반환해준다. 주로 게시판이나 회원가입시 날짜를
          입력하기 위해 사용된다. 오라클의 기본 서식이 xx/xx/xx이므로 출력시 시간은
          표시되지 않지만 서식문자를 통해 초단위까지 표현할 수 있다.*/
...
/*
-(하이픈), /(슬러시)와 같은 특수기호 외에는 서식으로 인식하지 못하므로 이를 제외한 나머지
문자열은 "로 묶어줘야한다. 서식문자를 감싸는게 아니므로 작성에 주의해야한다.
*/
...
select 12345,
    ...
    to_char(123, '999,999'), ltrim(to_char(12345,'999,999')),
...
*/
--문자열 사이에 ,가 포함되어 있으므로 숫자로 변환할 수 없어 에러 발생
select to_number('123,000')+10 from dual;
--replace()로 ,를 제거한 후 숫자로 변환하고 덧셈을 진행한다.
select to_number(replace('123,000', ',' ,''))+10 from dual;

select to_char(to_number(replace('123,000', ',' ,''))+10,'000,000') from dual;

/*
to_date()
    문자열 데이터를 날짜형식으로 변환해서 출력해본다. 기본서식은 년/월/일 순으로 지정된다.
*/
--하이픈, 슬러시와 같은 문자로 구분되어있는 경우 기본서식으로 인식한다.
--따라서 별도의 서식문자 없이도 날짜로 인식할 수 있다.
select to_date('2025-04-02'),
    to_date('2025/04/02'),
    to_date('20250402') 
from dual;

--문자 형식의 날짜인 경우 아래와 같이 연산이 불가능하다
select '2025-04-02'+1 from dual; --error

/*
날짜를 통해 연산을 하고 싶다면 아래와 같이 날짜 변환함수를 사용해야한다.
날짜에 1을 더하는 것은 내일의 날짜를 인출한다.*/
select to_date('2025-04-02')+1 from dual;

/*
아래와 같이 날짜포멧이 년-월-일 이 아닌 경우에는 오라클이 인식하지 못하므로 에러가 발생한다.
이때는 날짜서식을 이용해서 오라클이 날짜로 인식할 수 있도록 처리해야한다.*/
select to_date('02-04-2025') from dual; --error
...
*/
select
    to_date('14-10-2021','dd-mm-yyyy')+1 as "1일후",
    to_date('04-19-2022','mm-dd-yyyy')+7 as "일주일후"
from dual;
...
/*
방법2 : 날짜와 시간의 서식을 적용한다. 특히 시간은 15시이므로 24시간제인 서식을 이용해야한다.
*/
select
...
select to_char(to_date('2021년01월01일','yyyy"년"mm"월"dd"일"'),
           'yyyy"년"mm"월"dd"일"dy"요일"') as "YY/MM/DD/DAY",
    to_char(to_date('2021년01월01일','yyyy"년"mm"월"dd"일"'), 'dy"요일"') as "요일"
from dual;
