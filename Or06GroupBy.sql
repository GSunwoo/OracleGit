...
/*
distinct : 동일한 값이 있는 경우 중복된 레코드를 제거한 후 하나의 레코드만 가져와서
           보여준다. 순수한 하나의 레코드이므로 통계적인 값을 계산할 수 없다.
*/
...
select 
    ...
    trim(to_char(sum(salary), '999,000')) as "sumSalary22"
from employees;
...
/*
count() : 그룹화된 레코드의 갯수를 카운트할 때 사용하는 함수
*/
...
/*
    ...
    * 사용을 권장한다. 컬럼의 특성혹은 데이터에 따른 방해를 받지 않으므로 실행속도가 빠르다.
*/
...
select
    count(*) "전체사원수",
    sum(salary) "급여합계",
    floor(sum(salary)/count(*))"평균급여(직접계산)",
...
/*2. 컬럼 자체의 값을 대문자로 변환 한 후 쿼리의 조건으로 사용한다.
인출된 결과로 80번 부서라는 것을 알 수 있다.*/
select *
    from departments 
...
select
    ltrim(to_char(avg(salary),'$990,000.0')) as "영업팀 평균급여"
    from employees
...
/*
아래 쿼리문은 에러가 발생된다. 그룹함수는일반컬럼에 바로 사용할 수 없다.
이와같은 경우에는 뒤에서 학습할 '서브쿼리'를 사용해야한다.*/
select first_name, salary from employees where salary=min(salary);

--전체 사원 중 가장 낮은 급여는 얼마인가요? 즉 급여의 최소값이 얼마인가요?
select min(salary) from employees;
--따라서 2100$를 받은 직원을 인출하면 위 질문을 해결할 수 있다.
...
select department_id, count(*)"사원수", to_char(sum(salary),'999,000')"급여총합", to_char(avg(salary),'999,000')"평균급여"
    from employees
    group by department_id
...
*/
/*같은 부서에서 근무하더라도 담당업무는 다를 수 있으므로 이 문제에서는 group by 절에 2개의
컬럼을 명시해야한다. 즉 부서로 그룹화 한 후 다시 담당업무별로 그룹화한다.*/
select 
    department_id, job_id, count(*)"사원수",to_char(avg(salary),'999,000')"평균급여"
    from employees
/*  where count(*) > 10 이 부분에서 에러 발생
    --> 담당업무별 사원수는 물리적으로 존재하는 컬럼이 아니므로 where절에 추가하면 에러가
        발생한다. 이 경우에는 having절에 조건을 추가해야한다.
        ex) 급여가 300인 사원 => 물리적으로 존재하므로 where절에추가
...
*/
select
    department_id, count(*) ClerkCnt, ltrim(to_char(avg(salary),'$999,000.00')) AvgSalary
from employees 
group by department_id
