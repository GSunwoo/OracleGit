...
begin
    ...
    조건을 판단하여 하나의 값을 할당하는 방식이라는 것이다.
...
반복문2 : while문
    ...
    반복의 조건이 있으므로 특벌한 경우가 아니라면 exit를 사용하지 않아도 된다.
*/
--예제12] 제어문(반복문 : while)
...
begin
    ...
    for num2 in 0 .. 10 loop
        dbms_output.put_line('for문'||num2);
    end loop;
end;
/
/* 변수 선언이 필요없다면 declare는 생략할 수 있다. 
   for문에 reverse를 추가하면 거꾸로 반복할 수 있다. */
begin
    for num2 in reverse 0 .. 10 loop
        dbms_output.put_line('리버스for문'||num2);
    end loop;
end;
/
...
declare
    result1 number;
begin
    for num1 in 2 .. 9 loop
        dbms_output.put_line(num1||'단');
        for num2 in 1 .. 9 loop
            result1 := num1 * num2;
            dbms_output.put_line(num1 ||'*'|| num2 ||'='|| result1);
        end loop;
...
begin
    for num1 in 2 .. 9 loop
        ...
        for num2 in 1 .. 9 loop
            ...
            resultStr := resultStr ||' '|| num1 ||'*'|| num2 ||'='|| result1;
        end loop;
        --하나의 단이 완성된 문자열을 출력
...
/*
커서(Cursor)
: select 쿼리에 의해 여러행의 반환되는 경우 각 행에 접근하기 위한 객체
선언방법]
    Cursor 커서명 Is
        select 쿼리문. 단 into절이 없는 형태로 기술한다.
    Open 커서명
        : 쿼리를 실행하라는 의미. 즉 Open할 때 Cursor선언시 select문이
    ...
    Cursor의 속성
        %Found    : 가장 최근의 인출(Fetch)이 행을 반환하면 True, 아니면 False
        %NotFound : %Found의 반대의 값을 반환한다.
        %RowCount : 지금까지 반환된 행의 수를 반환한다.
*/
...
declare
    ...
    /* 커서선언 : 부서테이블의 모든 레코드를 조희하는 select문으로
...
begin
    ...
    /*
    결과셋의 개수는 조건에 따라 다를 수 있으므로 Basic loop를 통해
    반복 인출한다. */
    loop
...
begin
    ...
    loop
        --인출한 정보를 참조변수에 저장
        fetch curEmp into
            varEmp.employee_id, varEmp.last_name, varEmp.salary;
        --인출할 정보가 없다면 루프탈출
    ...
    dbms_output.put_line('인출된 행의 개수: ' || curEmp%rowcount);
    --커서를 닫아서 자원해제
    close curEmp;
end;
/
