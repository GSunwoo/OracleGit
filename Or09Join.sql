...
- 가장 많이 사용하는 조인문으로 테이블간에 연결조건을 모두 만족하는 레코드를 
  검색할때 사용한다.
- 일반적으로 기본기(Primary key)와 외래키(forein key)를 사용하여 join하는 경우가
  대부분이다.
- 2개의 테이블에 동일한 이름의 컬럼이 존재해야하고, 이 경우 '테이블명.컬럼명'과
  같이 기술한다.
...
/*
위의 첫번째 쿼리문을 실행하면 열의 정의가 애매하다는 에러가 발생된다. 부서번호를
뜻하는 department_id가 양쪽 테이블 모두에 존재하므로 어떤 테이블에서 가져와
인출해야할지 명시해야한다.*/
...
--5. join 쿼리문 작성
/* 양쪽 테이블에 동시에 존재하는 컬럼인 경우에는 반드시 테이블명이나 별칭을 명시해야한다.*/
select 
...
/*
2] outer join(외부조인)
outer join은 inner join과는 달리 두 테이블간의 조인조건이 정확히 일치하지 않아도 기준이 되는 테이블에서
레코드를 인출하는 방식이다.
...
*/
select 
    employee_id, first_name, last_name, hire_date, nvl(department_name,'부서없음') as "DEPT_NAME"
...
select
    ...
    case
        when d.department_id is not null then department_name
        else '<부서없음>'
    end as "DEPT_NAME", 
    hire_date
from employees e
...
order by employee_id;
/*
오라클 방식으로 변경시 outer join 연산자인 (+)를 생략하면 inner join이 되므로 작성에 주의해야한다.
*/

/*
3]self join(셀프조인)
셀프조인은 하나의 테이블에 있는 칼럼끼리 조인하는 경우에 사용한다.
즉 자신의 테이블과 조인을 맺는것이다.
셀프조인에서는 별칭이 테이블을 구분하는 구분자의 역할을 하므로 매우중요하다.
...
--1. Kimberely와 일반사원의 테이블로 분할
--2. Kimberely의 입사일보다 큰 데이터를 조건으로 추가
select
    clerk.first_name||' '||clerk.last_name "NAME", clerk.hire_date
...
/*
    ...
    using절에 사용된 컬럼은 Join이 되는 테이블 양쪽에 동시에 존재하는 컬럼인 것을 전재로
    작성되기 때문에 굳이 별칭을 사용할 필요가 없기 때문이다.
    즉 using은 테이블의 별칭 및 on절을 제거하여 보다 심플하게 join 쿼리문을 작성할 수 있게 해준다.
*/
-- 18개의 레코드 인출
select 
    first_name||' '||last_name "NAME", email,
...
*/
--2005년에 입사한 사원인출(결과 : 29명)
select first_name, hire_date from employees
    where to_char(hire_date, 'yyyy')='2005';
select first_name, hire_date from employees
    where substr(hire_date, 1,2)='05';
-- 문제에서 주어진 지역정보로 지역일련번호를 확인 (결과 : 1500)
...
*/
select
    employee_id "사원번호", first_name "이름" , last_name "성", trim(to_char(salary,'$99,000')) "급여",
...
select
    employee_id "사원번호", first_name "이름" , last_name "성", trim(to_char(salary,'$99,000')) "급여",
    department_name "부서명", country_id "국가코드", COUNTRY_NAME "국가명"--, state_province, city, hire_date
/*from countries
...
from employees
    ...
    inner join countries
        using(country_id)
where
    hire_date like('05/%') and
    state_province = 'California' and
...
/*
    오라클 방식은 표준방식에서 inner join 대신 콤마를 이용해서 테이블을 조인하고 on절 대신 where절에 조인될 컬럼을 명시한다.
*/
...
/*
5. Equi Join(등가조인)을 사용하여 커미션(COMMISSION_PCT)을 받는 모든 사원의 이름, 부서명, 도시명을 출력하시오. 
...
/*
6. inner join과 using 연산자를 사용하여 50번 부서(DEPARTMENT_ID)에 속하는 모든 담당업무(JOB_ID)의 
고유목록(distinct)을 부서의 도시명(CITY)을 포함하여 출력하시오.
...
/*
7. 담당업무ID가 FI_ACCOUNT인 사원들의 매니져는 누구인지 출력하시오. 단, 레코드가 중복된다면 중복을 제거하시오. 
출력목록] 이름, 성, 담당업무ID, 급여
*/
select
    distinct mgr.first_name, mgr.last_name,
    mgr.job_id, mgr.salary
from employees clk, employees mgr
where clk.manager_id = mgr.employee_id
    and clk.job_id = 'FI_ACCOUNT';
    /*
    사원의 매니저ID와 매니저의 사원ID를 셀프조인의 조건으로 사용
    사원의 담당업무를 조건으로 추가
    */
/*
...
*/
select
    to_char(hire_date,'yyyy') hire_year, avg(salary)
