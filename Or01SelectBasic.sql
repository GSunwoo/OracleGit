...
설명 : select, where, order by 등 가장 기본적인 SQL문 사용해보기
*/
...
/*
AS(알리아스) : 테이블 혹은 컬럼에 별칭(별명)을 부여할때 사용. 내가 원하는 이름(영문,한글)
             으로 변경한 후 결과를 인출할 수 있다.
             활용] 급여+성과급율 => SalComm 과 같은 형태로 별칭을 부여함
*/
-- 별칭은 한굴로 설정할 수도 있다.
...
SELECT employee_id "사원번호", first_name "이름", last_name "성"
    FROM employees WHERE first_name='William';
/*
단, 레코드인 경우 대소문자를 구분한다. 따라서 아래 SQL문을 실행하면 에러가 발생하진
않지만 아무런 결과도 인출되지 않는다.*/
select employee_id "사원번호", first_name "이름", last_name "성"
    from employees where first_name='william';
...
/*
in연산자
: or 연산자와 같이 하나의 컬럼에 여러개의 값으로 조건을 걸고 싶을때 사용
==> 급여가 4200, 6400, 8000인 사원의 정보를 인출하시오.*/
-- 방법1 : or을 사용. 이 경우 컬럼명을 반복적으로 사용해야 하므로 불편하다.
...
*/ 
select * from employees where hire_date like('__/02/__');
select * from employees where hire_date like('___02___');
select * from employees where hire_date like('%_02_%');

/*
레코드 정렬하기(Sorting)
    오름차순 정렬 : order by 컬럼명 asc(혹은 생략가능)
    내림차순 정렬 : order by 컬럼명 desc
2개 이상의 컬럼으로 정렬해야 할 경우 ,로 구분해서 정렬한다.
이 경우 첫번째 컬럼으로 정렬된 상태에서 두번째 컬럼이 정렬된다.*/

/*
시나리오] 사원정보 테이블에서 급여가 낮은 순서대로 오름차순으로 정렬하여 인출하시오*/
select first_name, salary, email, phone_number from employees order by salary asc;
select first_name, salary, email, phone_number from employees order by salary; --오름차순 정렬의 경우 asc를 생략할 수 있다.
...
: 값이 null이거나 null이 아닌 레코드 인출.
  컬럼 중 null값을 허용하는 경우 값을 입력하지 않으면 null값이 입력되는데
  이를 대상으로 select할때 사용된다.*/
