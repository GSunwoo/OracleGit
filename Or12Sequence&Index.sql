...
형식] create sequence 시퀀스명
        ...
        Minvalue N      -> 시퀀스 최소값
        Maxvalue N      -> 시퀀스 최대값 
        Cycle 혹은 noCycle -> 최대 값에 도달한 경우 처음부터 다시 시작할지 여부 결정
        Cache 혹은 noCache -> cache 메모리에 오라클 서버가 시퀀스 값을 할당할지 여부를 지정
*/     
...
/*
다음 입력할 시퀀스를 반환한다. 실행할대마다 설정한 증가치만큼
증가된 값이 반환된다. 
*/
...
/*단, 시퀀스의 cycle 옵션에 의해 최대값에 도달하면 다시 처음부터
  ...
  즉, PK 컬럼에 사용할 시퀀스는 cycle 옵션을 사용하지 않아야한다.*/
...
alter sequence seq_serial_num
    increment by 1
    minValue 9  /*최소값은 현재의 값보다 크게 줄 경우 에러 발생됨.*/
