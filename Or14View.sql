...
/*
select 쿼리문 실행시 해당 테이블이 존재하지 않는다면 
"테이블 또는 뷰가 존재하지 않음"이라는 오류 메세지가 뜨게된다.
*/
select * from member;

/*
View 생성
형식] create [or replace] view 뷰이름 [(컬럼1, 컬럼2...ㅜ)]
      as
...
*/
create or replace view view_employees(id, fname, lname, jobid, hdate, deptid)
as
select
    employee_id , first_name, last_name, 
    job_id, hire_date, department_id
...
*/
/*
1. select문 작성. null이 있는 경우 사칙연산이 되지 않으므로 nvl()함수를 통해 0으로 변환 후
연산식을 작성해야한다.*/
select
...
select
    ...
    department_name, to_char(hire_date,'yyyy"년"mm"월"dd"일"'), state_province||'주의 '||city
from employees
    inner join departments using(department_id)
    inner join locations using(location_id);
-- 뷰 확인(복잡한 쿼리문을 view를 통해 간단히 조회할 수 있다.)
