...
declare
    ...
    '이름(name)의 경우에는 2개의 컬럼이 합쳐진 상태이므로 조금더 넉넉한 크기로
    설정해주는 것이 좋다.*/
    empName varchar2(50);
...
/*
    PL/SQL 문장의 끝에는 반드시 /를 붙여야하는데, 만약 없으면 호스트환경으로
    빠져나오지 못한다. 즉 PL/SQL문장을 탈출하기 위해 필요하다.
    호스트환경이랑 쿼리문을 입력하기 위한 SQL> 프롬프트가 있는 상태를 말한다.
*/

--예제3] 참조변수1(하나의 컬럼 참조)
/*
    참조변수 : 특정 테이블의 컬럼을 참조하는 변수로서 동일한 자료형과 크기로
               선언하고 싶을때 사용한다.
         형식] 테이블명.컬럼명%type
...
declare
    ...
    empSal employees.salary%type;        --NUMBER(8,2)로 선언
    deptId employees.department_id%type; --NUMBER(4,0)로 선언
begin
...
--예제4] 참조변수2(전체컬럼을 참조)
/*
시나리오] 사원번호가 100인 사원의 레코드를 가져와서 emp_row변수에 전체컬럼을 저장한
후 화면에 다음 정보를 출력하시오. 단, emp_row는 사원테이블이 전체컬럼을 저장할 수 
있는 참조변수로 선언해야한다. 
...
/*
복합변수
: class를 정의하듯 필요한 자료형을 하나로 묶어 새로운 자료형을 만든 후
  생성하는 변수를 말한다.(구조체와 유사)
  형식]
...
declare
    --3개의 값을 지정할 수 있는 복합변수 자료형을 선언한다.
...
1. 복합변수생성
   ...
   - 복합변수자료형의 이름 : empTypes
     ...
     위에서 생성한 자료형을 이용하여 복합변수 rec2를 생성후 사원번호 108번의 정보를 
     할당한다.
...
begin
    ...
    select 
        ...
        salary, nvl(commission_pct, 0)
            into rec2
    from employees where employee_id = 108/*&searchid*/;
...
치환연산자 : PL/SQL에서 사용자로부터 데이터를 입력받을때 사용하는
             연산자로 변수 앞에 &를 붙여주면 된다. 실행시 입력창이 뜬다.
*/
/* 시나리오] 앞에서 작성한 프로그램을 치환연산자로 사원번호를 입력받을 수 있도록
   수정하시오. */ 
declare
    type empTypes is record(     
        emp_id employees.employee_id%type,
...
/*
바인드 변수
    : 호스트환경에서 선언된 변수로서 비PL/SQL 변수이다.
      호스트환경은 SQL> 프롬프트가 있는 상태로 명령어(쿼리문)를
      입력할 수 있는 상태를 말한다.
...
declare
    --선언부에는 이와같이 아무 내용이 없을 수도 있다.
begin
    --바인드변수는 일반변수와 구분을 위해 :을 추가한다.
    :return_var := 999;
...
/*
    ...
    CMD에서는 개별적으로 실행해도 문제가 없지만, developer에서는
    바인드변수부터 마지막 print까지 묶은 후 실행해야 결과가 제대로 출력된다.
*/
