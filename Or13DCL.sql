...
*/
 ...
 : 여러 사용자가 다양한 권한을 효과적으로 관리할 수 있도록 관련된
   권한끼리 묵어놓은 것을 말한다.
※ 우리는 실습을 위해 새롭개 생성한 계정에 connect, resource 를 주로 부여한다.
 */

-두번째 계정을 생성한 후 role을 동태 권한을 부여한다.
create user test_user2 identified by 1234;
/*
아래 2개의 Role은 오라클에서 기본적으로 제공된다. 접속 및 테이블 생성 등의
대부분의 기본작업을 할 수 있는 권한이 포함되어있다.
*/
...
/*
    접속권한 회수 후 접속을 시도할 때 비밀번호가 틀리면 '부적합' 에러가 발생되고
...
※ cascade를 명시히면 사용자계정과 관련된 모든 데이터베이스 스키마가 데이터사전으로부터 삭제되고
  모든 스키마 객채도 물리적으로 삭제된다.
*/
