...
select * from tab;
--물리적으로 존재하는 테이블의 경우 정상적인 레코드가 인출된다.
...
/*
Dual테이블
: 하나의 행으로 결과를 출력하기 위해 제공되는 테이블로 오라클에서 자동으로 생성되는
  논리적 테이블이다. varchar2(1)로 정의된 dummy라는 단 하나의 컬럼으로 구성되어있다.*/
select * from dual;
...
select abs(salary) as "급여의 절대값" from employees;

/*
trunc() : 소수점을 특정자리수에서 잘라낼때 사용
    형식] trunc(컬러명 혹은 값, 소수점이하 자리수)
         두번째 인수가
            양수 : 주어진 숫자만큼 소수점 표현
...
select first_name, salary, trunc(commission_pct,1), salary + (salary*trunc(commission_pct,1)) as salcom
    from employees where job_id like 'SA_%';
